...
var polygonLineBuffer = function (_BufferBase) {
      ...
      value: function geometryBuffer() {
        ...
        coordinates.forEach(function (geo, index) {
          ...
          if (isExtrude && shape === 'extrudeline') {
            extrudeData = __WEBPACK_IMPORTED_MODULE_0__shape_index__['b'].extrudeline(geo);
            extrudeData.positions = extrudeData.positions.map(function (pos) {
              pos[2] *= heightValue;
              return pos;
            });
          } else {
            extrudeData = __WEBPACK_IMPORTED_MODULE_0__shape_index__['b'].line(geo);
          }
